    ...
    def self.view_list_distinct_user_checkins_all(params = {})
        ...
        @checkins.map do |user| 
            ...
            next unless user.profile 
            {
    			...
    			"hours_since_checkin" => ((Time.now - user.check_ins.last.check_in_time)/3600).to_f.round,
    			"minutes_since_checkin" => ((Time.now - user.check_ins.last.check_in_time)/60).to_f.round,
